...
-- Дамп структуры для процедура master_v_balashihe.blog_article_keyTextHtml_save_draft
DELIMITER //
CREATE PROCEDURE `blog_article_keyTextHtml_save_draft`(
...
BEGIN
	...
	SELECT draftArticleId INTO stored_draftArticleId FROM draft_blog_article d
		WHERE in_managerId=d.managerId AND d.isDraftSaved IS NULL And
				...
				IFNULL(in_keyTextHtml, '')=IFNULL(d.`keyTextHtml`,'');
	...
	Else
		...
		Then
			...
			ELSE 	
				INSERT INTO draft_blog_article(existingArticleId, managerId, `keyTextHtml`)
					VALUES(
						...
						If(in_keyTextHtml='' OR in_keyTextHtml IS NULL,NULL,TRIM(in_keyTextHtml))
					);
		...
		Else
			UPDATE draft_blog_article SET 
				`keyTextHtml`=If(in_keyTextHtml='' OR in_keyTextHtml IS NULL,NULL,TRIM(in_keyTextHtml))
