	...
	Else
		...
		Then
			INSERT INTO draft_blog_article(existingArticleId, managerId, title, handle, autoHandleSlug, blogCategoryId)
				VALUES(				
					...
					If(in_title='' OR in_title IS NULL,NULL,TRIM(in_title)),
					If(in_handle='' OR in_handle IS NULL,NULL,TRIM(in_handle)),
					If(in_autoHandleSlug='' OR in_autoHandleSlug IS NULL,NULL,TRIM(in_autoHandleSlug)),
					If(in_blogCategoryId='' OR in_blogCategoryId IS NULL,NULL, in_blogCategoryId)
				);
		...
		Else
			UPDATE draft_blog_article SET 
				title=If(in_title='' OR in_title IS NULL,NULL,TRIM(in_title)),
				handle=If(in_handle='' OR in_handle IS NULL,NULL,TRIM(in_handle)),
				autoHandleSlug=If(in_autoHandleSlug='' OR in_autoHandleSlug IS NULL,NULL,TRIM(in_autoHandleSlug)),
				blogCategoryId=If(in_blogCategoryId='' OR in_blogCategoryId IS NULL,NULL, in_blogCategoryId)
