...
BEGIN
   ...
   Then
      UPDATE contact_emails e SET e.counts=COALESCE(e.counts,1)+1
         WHERE e.timestamp=client_timestamp AND e.ip=in_ip;
   ...
   Then
      UPDATE contact_emails e SET e.counts=COALESCE(e.counts,1)+1
           WHERE e.createdAt >= SUBDATE(NOW(),1) And e.valuesText=in_valuesText;
