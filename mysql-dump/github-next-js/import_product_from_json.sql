...
DECLARE productId INT Unsigned DEFAULT NULL;
...
If product_handle IS NOT NULL And NOT EXISTS(SELECT 1 FROM product c WHERE c.handle = product_handle)
Then 
	...
	DO
		CALL save_product_variant(product_variant, productId);
		Set i := i + 1;
		SELECT JSON_EXTRACT(in_product_variants,CONCAT('$[',i,']')) INTO product_variant;
	END WHILE;
	...
	DO
		...
		IF out_imageId Is NOT NULL AND NOT EXISTS (SELECT 1 FROM image_to_product WHERE image_to_product.productId=productId AND image_to_product.imageId=out_imageId)
		Then
			INSERT INTO image_to_product(`productId`, `imageId`, `orderNumber`) VALUES(productId, out_imageId, i);
		END If;
		Set i := i + 1;
		SELECT JSON_EXTRACT(in_product_images, CONCAT('$[',i,']')) INTO in_image;
	END WHILE;
