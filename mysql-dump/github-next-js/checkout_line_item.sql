...
CREATE TABLE IF NOT EXISTS `checkout_line_item` (
  ...
  CONSTRAINT `FK_checkout_line_items_checkout` FOREIGN KEY (`checkoutId`) REFERENCES `checkout` (`checkoutId`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Дамп данных таблицы github-next-js.checkout_line_item: ~0 rows (приблизительно)
