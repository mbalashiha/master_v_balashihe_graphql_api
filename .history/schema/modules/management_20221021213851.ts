  ...
  resolvers: {
    UploadedImagesNodes: {
      nodes: async (parent, variables, context, info: GraphQLResolveInfo) => {
        ...
        try {
          ...
          } else if (productId) {            images = await db.excuteQuery({
              query: `select i.*, i.originalSrc as imgSrc, ip.orderNumber
                 from draft_product p
                 Left JOIN draft_image_to_product ip On p.draftProductId=ip.draftProductId
                 Left JOIN draft_image i On ip.draftImageId=i.draftImageId
            where p.managerId=$managerId And p.productId=$productId 
                And p.updatedAt in (select Max(updatedAt) from draft_product Where managerId=$managerId And productId=$productId Group By updatedAt)
            order By ip.orderNumber`,
              variables,
            });
          };
    ...
    UploadedImagesResponse: {
      imagesConnection: async (
          ...
          if (!variables.draftProductId) {
            ...
            if (!variables.productId) {
              rows = await db.excuteQuery({
                query: `SELECT lcase(hex(d.draftProductId)) as draftProductId FROM draft_product d 
	 	WHERE d.managerId=$managerId AND d.productId IS NULL
		ORDER BY d.updatedAt DESC LIMIT 1`,
              ...
              rows = await db.excuteQuery({
                query: `SELECT lcase(hex(d.draftProductId)) as draftProductId  FROM draft_product d 
	 	WHERE d.managerId=$managerId AND d.productId=$productId
		ORDER BY d.updatedAt DESC LIMIT 1`,
                variables,
              });
            }
            variables.draftProductId =
              (rows && rows[0] && rows[0].draftProductId) || null;
          }
          return variables;
        } catch (e: any) {
          console.error(e.stack || e.message);
    ...
    Mutation: {
      ...
      removeProductImage: async (
        ...
        try {
          let {
            imagesInput: { draftProductId, productId, images },
          } = variables;
          ...
          if (!draftProductId) {
            try {
              ...
              draftProductId = preDraft[0][0].draftProductId;
            } catch (e: any) {
          ...
          return { removedImages, draftProductId };
        } catch (e: any) {
          console.error(e.stack || e.message);
          debugger;
          throw e;
      ...
      productImagesUpdate: async (
        ...
        try {
          const {
            imagesInput: { productId, draftProductId, images },
          } = variables;
          if (!Array.isArray(images) || images.length <= 0) {
            throw new Error(
              "No variable images! Should be array of images input."
