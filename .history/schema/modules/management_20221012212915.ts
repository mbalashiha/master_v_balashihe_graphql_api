        ...
        try {
          ...
          if (draftProductId && productId) {
            ...
            where p.managerId=$managerId And p.productId=$productId 
                And p.updatedAt in (select Max(updatedAt) from draft_product Where managerId=$managerId And productId=$productId Limit 1)
            order By ip.orderNumber`,
              variables,
            });
            if (!images[0]) {
            ...
            where p.managerId=$managerId And p.productId is Null and 
                And p.updatedAt in (select Max(updatedAt) from draft_product Where managerId=$managerId And productId is Null Limit 1)
            order By ip.orderNumber`,
              variables,
            });
          }
